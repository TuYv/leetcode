...
class Solution {
	public int myAtoi(String str) {
		...
		int rox = 0, pop = 0, flag = 0, index = 0;
		boolean b = false, x = true;
		...
		for(int i = index; i < chars.length; i++) {
			...
			if ((47 < c && 58 > c)) {
				...
				if (rox > Integer.MAX_VALUE / 10 || (rox == Integer.MAX_VALUE / 10 && pop > 7))
					return Integer.MAX_VALUE;
				if (rox < Integer.MIN_VALUE /10 || (rox == Integer.MIN_VALUE / 10 && pop < -8 ))
